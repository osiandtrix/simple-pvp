...
pub fn fetch_settings() -> Result<Settings, String> {
    db::with_conn(|conn| {
        let mut stmt = conn.prepare("SELECT min_level, max_level, api_key, always_on_top, keybinds_enabled FROM settings LIMIT 1")?;
        stmt.query_row([], |row| {
            Ok(Settings {
...
#[tauri::command]
pub fn update_settings(min_level: Option<i64>, max_level: Option<i64>, api_key: Option<String>, keybinds_enabled: Option<bool>) -> Result<(), String> {
    db::with_conn(|conn| {
